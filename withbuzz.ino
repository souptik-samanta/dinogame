...
#define PIN_BUZZER 11  // Define the buzzer pin
...
// Initialize the I2C LCD with the I2C address (usually 0x27 or 0x3F) and dimensions
...
void playBuzzerSound(int duration) {
  tone(PIN_BUZZER, 1000); // 1000 Hz tone
  delay(duration);
...
void loop(){
  ...
  if (buttonPushed) {
    if (heroPos <= HERO_POSITION_RUN_LOWER_2) {
      ...
      playBuzzerSound(100); // Play sound when jumping
    }
    buttonPushed = false;
  }  

  if (drawHero(heroPos, terrainUpper, terrainLower, distance >> 3)) {
    playBuzzerSound(200); // Play sound when the hero collides
